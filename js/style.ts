...
function deriveStyle(styleSeedHex: string): StyleResult | null {
  if (!HEX64_RE.test(styleSeedHex)) return null;
...
function generateAvatarSvg(seed: string, color: string): string {
  const cells: number[] = [];
  for (let i = 0; i < 15; i++) {
